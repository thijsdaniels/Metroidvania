...
namespace Physics
{
    ...
    public class Body : MonoBehaviour
    {
        ...
        public PlatformParameters DefaultPlatformParameters;
        public PlatformParameters PlatformParameters => Platform ? Platform.Parameters : DefaultPlatformParameters;
        public float WallFriction = 0.2f;

        /// <summary>
        /// Volumes
        /// </summary>
        public Volume Volume { private get; set; }
        private Volume PreviousVolume;
        public VolumeParameters DefaultVolumeParameters;
        public VolumeParameters VolumeParameters => Volume ? Volume.Parameters : DefaultVolumeParameters;
        ...
        public float FloatingPoint = 0.5f;
        private float BouyancyFactor = 1f;
...
//            ApplyBouyancy();
        ...
        /// TODO: This is Traits.Jumper logic and has no place in the Physics.Body.
        ...
        /// 
        /// </summary>
        /// TODO: This method doesn't work, because the raycast will never collide from inside the volume...
        private void ApplyBouyancy()
        {
            if (!Volume || !Volume.IsLiquid)
            {
                return;
            }
            
            RaycastHit2D raycastHit = Physics2D.Raycast(transform.position, Vector2.down, 1f, VolumeLayerMask);
            float surfaceDistance = raycastHit.distance;

            if (!raycastHit)
            {
                float bouyancy = (Density - Volume.Density) * surfaceDistance;
            
                AddVerticalVelocity(bouyancy * BouyancyFactor);
            }
        }
            ...
            for (int i = 0; i < VerticalRays; i++)
            {
                ...
                if (!movingUp)
                {
                    ...
                    if (verticalDistanceToHit < standingOnDistance)
                    {
                        ...
                        Platform = raycastHit.collider.gameObject.GetComponent<Platform>();
                    }
                }
        ...
        private void DeterminePlatform()
        {
            if (Platform != null)
            {
                ...
                {
                    SendMessage("OnLand", Platform, SendMessageOptions.DontRequireReceiver);
                }
                ...
                if (PreviousPlatform != Platform)
                {
                    if (PreviousPlatform != null)
                    {
                        PreviousPlatform.SendMessage("OnBodyExit", this, SendMessageOptions.DontRequireReceiver);
                    }

                    Platform.SendMessage("OnBodyEnter", this, SendMessageOptions.DontRequireReceiver);
                    PreviousPlatform = Platform;
                }
                else
                {
                    Platform.SendMessage("OnBodyStay", this, SendMessageOptions.DontRequireReceiver);
                }
            }
        ...
        private void DetermineVolume()
        {
            if (Volume != null)
            {
                if (PreviousVolume != Volume)
                {
                    ...
                    {
                        PreviousVolume.SendMessage("OnBodyExit", this, SendMessageOptions.DontRequireReceiver);
                        SendMessage("OnVolumeExit", PreviousVolume, SendMessageOptions.DontRequireReceiver);
                    }

                    Volume.SendMessage("OnBodyEnter", this, SendMessageOptions.DontRequireReceiver);
                    SendMessage("OnVolumeEnter", Volume, SendMessageOptions.DontRequireReceiver);
                    
                    PreviousVolume = Volume;
                }
                else
                {
                    Volume.SendMessage("OnBodyStay", this, SendMessageOptions.DontRequireReceiver);
                    SendMessage("OnVolumeStay", Volume, SendMessageOptions.DontRequireReceiver);
                }
            }
        ...
        {
            other.SendMessage("OnBodyEnter", this, SendMessageOptions.DontRequireReceiver);
        }
        ...
        {
            other.SendMessage("OnBodyStay", this, SendMessageOptions.DontRequireReceiver);
        }
        ...
        {
            other.SendMessage("OnBodyExit", this, SendMessageOptions.DontRequireReceiver);
        }
